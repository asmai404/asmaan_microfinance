...
function display_recent_transactions() {
    global $conn;

    // Call the function to get all loanees
    $names = get_recent_transactions();
    ...
    if ($names !== false) {
        ...
        if (!empty($names)) {
            ...
            echo "<table>";
            echo "<tr style='color:#0056b3;'><th>Client name</th><th>Client email</th><th>Loan amount</th><th>Outstanding balance</th><th>Total amount repaid</th><th>Current amount repaid</th><th>Transaction date</th><th>Status||Actions</th></tr>";         
		...
		// Determine status color and status title based on status (update)
                ...
                echo "<a href='../admin/update_payment_verification_status.php?t_id=" . $name['t_id'] . "' class='delete-link'>";
                echo "<i class='fas fa-check-circle' title='" . $statusTitle . "' style='color: " . $statusColor . "' ></i>";
                echo "</a>&nbsp;&nbsp;&nbsp;";
		echo "<a href='../admin/edit_outstanding_balance.php?t_id=" . $name['t_id'] . "' class='edit-link' style='color:#00cc00;'><i class='fas fa-edit' title='Edit'></i></a>";
                echo "</td>";
